...
int main(int argc, char *argv[]){
	...
	printf("\n  Al segmento le ha gustado :-)\n");
}

//Esta es la funcion q funciona!!
...
void *copy_directorio(void *arg){
	...
	//Desaprovechamos espacio  
	char *origen=(char *)malloc(sizeof(arg));	//Como max seran del tam d arg
	char *destino=(char *)malloc(sizeof(arg));
	char *nb, *arg2, *rutac, *ruta2, *arg3, *buf;
	int tipo, n, i;
	...
	*destino='\0';
	...
	while ( (direntp=readdir(dirp)) != NULL){
		...
		lstat(rutac, &buffer); //Recibimos info del fichero
		...
		switch (tipo){
			case S_IFREG:
				...
				if (pthread_create(&hilo2, NULL, copia_fichero, (void *)arg2) < 0)
			...
			case S_IFDIR:
				...
				printf("Hilo numero %d comienza a copiar directorio %s\n",pthread_self(),ruta2);
				break;
			case S_IFSOCK:
			...
			case S_IFLNK:
				...
				i = 0;
				buf = NULL;
				/* lectura de la ruta a la que apunta el enlace */
				do {
					if (n == i) {
						n += 256;
						buf = realloc(buf, n);
						...
						break;
				} while (i = readlink (rutac, buf, n));
				if (i == -1) {
					fprintf(stderr,"Error al leer el enlace simbolico %s\n",rutac);
					free(buf);
					break;
				}
				/* creación del enlace */
				if (symlink (buf, ruta2)) {
					fprintf(stderr,"Error al crear el enlace simbolico %s\n",ruta2);
				}
