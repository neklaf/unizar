...
var ENDSHOW_MESG="Fin de la presentaci—n, haga clic para salir.", SCREEN_MODE="Frames", gIsEndShow=0, NUM_VIS_SLDS=22, SCRIPT_HREF="script.js", FULLSCR_HREF="fullscreen.htm";
...
var sNext="Siguiente",sPrev="Anterior",sEnd="Fin de la presentacion",sFont="Arial", alwaysOn = false
function ShowMenu()
{
	BuildMenu();
...
var g_baseURL = gtmpstr.substr(0, gtmpstr.lastIndexOf("/") ) + "/" + "SchedulingAS_archivos";
var g_showoutline = 1;
  ...
  if ( (msie || isnav ) && ( (os < 0 && ver >= 4 ) || ( os >= 0 && ver >= 5 ) || (os >=0 && msie < 0 && ver >= 4 ) )  ){
	if ( msie >= 0 )  {
		if ( isIEFiles > 0 )
			...
			/*if ( IsFramesMode() )
				StatusPlay("Esta presentacion esta creada para utilizarla con versiones anteriores del explorador. Puesto que esta utilizando una version mas moderna de Microsoft Internet Explorer o Netscape Navigator, puede optimizar la presentacion para sacar partido de las capacidades avanzadas de la version."); */
			jpegArray( 22 );
		}
		else
			window.location.replace(  "SchedulingAS_archivos/error.htm" );
	}
	else {
	...
	if ( IsFramesMode() && !isWebTV() )
		StatusPlay("Esta presentacion contiene elementos que el explorador no puede mostrar correctamente. Esta version esta creada para utilizar versiones mas modernas de Microsoft Internet Explorer o Netscape Navigator.");
	*/
	if ( isFLATFiles <= 0 ) {
	/*	if ( IsFramesMode() )
			window.alert("Esta presentacion contiene elementos que el explorador no puede mostrar correctamente. Esta version esta creada para utilizar versiones mas modernas de Microsoft Internet Explorer o Netscape Navigator." );
	*/
		window.location.replace(  "SchedulingAS_archivos/error.htm" );
	}		
	/*
