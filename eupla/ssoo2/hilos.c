	...
	*destino='\0';
	...
	if ( (dirp2 = opendir(destino)) == NULL){
		...
		if (errno == ENOENT){
			...
			//Tenemos q preguntar por el modo del directorio nuevo
			mkdir(destino,0777);	
		}
	}
	//Esto se puede hacer con la funcion seekdir!!
	...
	while ( (direntp=readdir(dirp)) != NULL){
		...
		/*S_ISDIR(st_mode) macro utilizada en POSIX para ver si es un 
		directorio o no*/
		/*Hay q pasarle la ruta completa al stat para q lo coja bien el 
		archivo*/
